...
try:
    # Airflow 3.0 imports - updated paths and new structure
    from airflow.configuration import conf
    from airflow.models import TaskInstance
    from airflow.utils.task_group import TaskGroup
    
    # Airflow 3.0: Use new provider paths for operators
    from airflow.providers.standard.operators.empty import EmptyOperator
    from airflow.providers.standard.operators.python import PythonOperator
    
    # Airflow 3.0: get_current_context moved to airflow.sdk
    try:
...
class PipelineTasksGroup(TaskGroup):
    """
    Represents a dlt Airflow pipeline task group for Airflow 3.0.
    """
    ...
    def _run(
        ...
        if self.use_task_logger:
            # Airflow 3.0: Simplified logging approach to avoid segmentation faults
            try:
                # Use Airflow's built-in logging system directly
                import logging
                logger.LOGGER = logging.getLogger("airflow.task")
            except Exception:
        ...
        **kwargs: Any,
        ...
        with self:
            ...
            def make_task(pipeline: Pipeline, data: Any, name: str = None) -> BaseOperator:
                # Defer data evaluation to runtime to avoid recursion during import
                def deferred_run():
                    # If data is a callable, call it at runtime
                    if callable(data):
                        actual_data = data()
                    else:
                        actual_data = data
                ...
                return PythonOperator(
                    task_id=self._task_name(pipeline, data), python_callable=deferred_run, **kwargs
                )
            ...
            if decompose == "none":
                ...
                # serialize tasks - handle decomposition more carefully to avoid recursion
                tasks = []
                pt = None
                try:
                    # Try to decompose the source
                    decomposed_sources = data.decompose("scc")
                    ...
                    # If recursion error occurs, fall back to no decomposition
                    logger.warning("Recursion error during source decomposition, falling back to no decomposition")
                    return [make_task(pipeline, data)]
                return tasks
                ...
                end = EmptyOperator(task_id=f"{t_name}_end", **kwargs)
...
def airflow_get_execution_dates() -> Tuple[pendulum.DateTime, Optional[pendulum.DateTime]]:
    ...
    try:
        # Airflow 3.0: Updated context access

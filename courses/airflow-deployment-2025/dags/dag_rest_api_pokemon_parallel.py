...
from airflow_helper3 import PipelineTasksGroup

# modify the default task arguments - all the tasks created for dlt pipeline will inherit it
# - set e-mail notifications
#clear - we set retries to 0 and recommend to use `PipelineTasksGroup` retry policies with tenacity library, you can also retry just extract and load steps
...
@dag(
    dag_id="pokemon_dag_parallel_3.0",
    schedule='@daily',
    start_date=pendulum.datetime(2023, 7, 1),
    catchup=False,
    max_active_runs=5,  # Keep this at 1 to avoid multiple DAG runs
